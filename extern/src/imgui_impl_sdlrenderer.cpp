...
// offers a limited graphic API to the end-user and it might be difficult to
// step out of those boundaries. However, we understand it is a convenient
// choice to get an app started easily.

// Implemented features:
//  [X] Renderer: User texture binding. Use 'SDL_Texture*' as ImTextureID. Read
//  the FAQ about ImTextureID! [X] Renderer: Support for large meshes (64k+
...
void ImGui_ImplSDLRenderer_NewFrame() {
  ...
  if (!bd->FontTexture) ImGui_ImplSDLRenderer_CreateDeviceObjects();
}
...
void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData* draw_data) {
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        const float* xy =
            (const float*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
        ...
        const SDL_Color* color =
            (const SDL_Color*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
                               IM_OFFSETOF(ImDrawVert, col));  // SDL 2.0.19+
#else
        const int* color =
            (const int*)((const char*)(vtx_buffer + pcmd->VtxOffset) +
  ...
  SDL_RenderSetClipRect(bd->SDLRenderer,
                        old.ClipEnabled ? &old.ClipRect : NULL);
}
